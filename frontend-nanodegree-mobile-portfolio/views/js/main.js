...
var resizePizzas = function(size) {
  ...
  // pizza size changes with slider scale
  //Changed all quertySelector to getElementId 
  function changeSliderLabel(size) {
    switch(size) {
      ...
      case "3":
        ...
        return;
      default:
        alert("problem with slider");
    }
  }

  changeSliderLabel(size);

  // each case has a var called "newWidth"
  //Switch the width value from pixie to Percents 
  function changePizzaSizes(size) {
    ...
    for (var i = 0; i < randomPizzas.length; i++) {
      randomPizzas[i].style.width = 100 * newWidth + "%";
    }
  }
...
console.log("Time to generate pizzas = " + timeToGenerate[0].duration + "ms");
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  ...
  console.log("time(average) for the last 10 frames = " + sum / 10 + "ms");
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  pizzasTotal = rows * cols;
  for (var i = 0; i < pizzasTotal; i++) {
